...
float distanceBetweenTwoPoints(t_point firstPoint, t_point secondPoint)
{
...
t_vector2 get_vector_from_points(t_point firstPoint, t_point secondPoint)
{
...
t_range add_new_float_to_range(t_range currentRange, float newPoint)
{
	t_range newRange = currentRange;
	if (newPoint >= newRange.max)
	{
		newRange.max = newPoint;
	}
	else if (newPoint <= newRange.min)
	{
		newRange.min = newPoint;
	}
